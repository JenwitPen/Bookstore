...
public class UserRepository {
	...
	public List<UserBookDB> findBookByUser(int userid) {
		return jdbcTemplate.query(
				"select u.id as userid,u.date_of_birth,u.name,u.surname,o.bookid  from user as u  join `order`  as o on u.id=o.userid where u.id=?",
	...
	public int insert(UserDB userdb) {
		return jdbcTemplate.update(
				"insert into user (name, surname, date_of_birth,username,password,createdate,createuser) "
						+ "values(?,?,?,?,?,?,?)",
				new Object[] { userdb.getName(), userdb.getSurname(), userdb.getDate_of_birth(), userdb.getUsername(),
						userdb.getPassword(), new Date(), "admin" });
	}
	...
	public double insertOrder(OrderDB orderdb) {
		int x = jdbcTemplate.update(
				"insert into `order` (bookid,userid,createdate,updatedate,createuser,updateuser,active) "
						+ "values(?,?,?,?,?,?,?)",
				new Object[] { orderdb.getBookid(), orderdb.getUserid(), new Date(), null, "admin", null, true });
		if (x > 0) {
			return jdbcTemplate.queryForObject("select * from book where id=?", new Object[] { orderdb.getBookid() },
					new BeanPropertyRowMapper<BookDB>(BookDB.class)).getPrice();
		} else {
			return 0;
		}
	}
