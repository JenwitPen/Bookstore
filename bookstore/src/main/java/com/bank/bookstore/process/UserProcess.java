...
public class UserProcess {
		...
		try {
			...
			for (UserDB user : uselist) {
				...
				ArrayList<Object> books = new ArrayList<Object>();	
					...
					UserOrderResponse userOrder = new UserOrderResponse(user.getName(), user.getSurname(), user.getDate_of_birth(), books);
	...
	public ResponseMessage addUserProcess(UserRequest userRequest) {
		try {
			UserDB userdb =new UserDB(userRequest.getName(), userRequest.getSurname(), userRequest.getDate_of_birth(), userRequest.getUsername(), userRequest.getPassword());
	...
	public ResponseMessage removeAllUserProcess() {
		try {
			return new ResponseMessage(true, userRepository.deleteAll(), null, HttpStatus.OK);
		} catch (Exception ex) {
			throw new java.lang.RuntimeException(ex);
		}
	...
	public ResponseMessage insertOrderProcess(UserOrderRequest userOrderRequest) {
		...
		Double price=0.0;
			...
			return new ResponseMessage(true, price, null, HttpStatus.OK);
		} catch (Exception ex) {
			throw new java.lang.RuntimeException(ex);
		}
